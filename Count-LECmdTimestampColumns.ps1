<#
	.SYNOPSIS
		A useful script to help identify how many unique values exist in SourceCreated and SourceModified columns with LECmd CSV output
	
	.EXAMPLE
		.\Count-LECmdTimestampColumns.ps1 C:\temp\20240606182010_LECmd_Output.csv

	.OUTPUTS
		Example output:
		
		Total count of values in SourceCreated column: 399
		Number of unique values in SourceCreated column: 324
		Total count of values in SourceModified column: 399
		Number of unique values in SourceModified column: 316
		Total unique values between both columns: 474
		Total number of values in both columns: 798
	
	.PARAMETER path
		Provide a file path for LECmd CSV output
	
	.NOTES
		===========================================================================
		Created with: 	SAPIEN Technologies, Inc., PowerShell Studio 2024 v5.8.243
		Created on:   	6/13/2024 22:10
...
(
	[Parameter(Mandatory = $true,
			   Position = 1,
			   HelpMessage = 'Provide a file path for LECmd CSV output')]
	[ValidatePattern('^.*_LECmd_Output\.csv$')]
	[string]$Path
)
...
## Get counts for SourceCreated column
$sourceCreatedTotal = ($csvData.SourceCreated).Count
$sourceCreatedUnique = ($csvData.SourceCreated | Select-Object -Unique).Count

## Get counts for SourceModified column
$sourceModifiedTotal = ($csvData.SourceModified).Count
$sourceModifiedUnique = ($csvData.SourceModified | Select-Object -Unique).Count

## Get total unique values between both columns
$uniqueValues = $csvData | Select-Object SourceCreated, SourceModified -Unique
$totalUniqueValues = ($uniqueValues.SourceCreated + $uniqueValues.SourceModified | Select-Object -Unique).Count

## Get total values in both columns
$totalValues = $sourceCreatedTotal + $sourceModifiedTotal

## Output results
Write-Host "Total count of values in SourceCreated column: $sourceCreatedTotal"
Write-Host "Number of unique values in SourceCreated column: $sourceCreatedUnique"
Write-Host "Total count of values in SourceModified column: $sourceModifiedTotal"
Write-Host "Number of unique values in SourceModified column: $sourceModifiedUnique"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCG4vCPaJhsUa7S
# 7DqEK9KBDr97MrTZleHbMP1dww2/1aCCKKMwggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCDu
# ZPPBvdMGFwfe9mkWFQXp5bso4gB9KA0cJnepLY9rwDANBgkqhkiG9w0BAQEFAASC
# AgAvH99j8OhyF9zC8Q3aDeACPQs9Xsjy6Omsqrxw9lAakxCheJDNEUJXiQCMs/Z1
# kVO9sXDf5OO+8fh+2cSNo9TiUYm5N1O1aq4gJA5xaDAjsr1oLi3NuF43iMsgl7aR
