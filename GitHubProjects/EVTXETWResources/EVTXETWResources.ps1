...
# Execute logman command to retrieve ETW providers
$logmanOutput = & logman query providers

Write-Host (Get-Date).ToString() "Running logman query providers to generate a list of Providers"

# Extract the provider names
$providerList = $logmanOutput | Select-String -Pattern '^\S+' | Select-Object -Skip 2 | ForEach-Object {
	$_.Matches.Value
}

$wepTool = 'C:\temp\cli.exe'

Write-Host (Get-Date).ToString() "Running cli.exe with each Provider name obtained by logman to generate a Provider XML"

# Pass each provider name as an argument to the external command-line tool
$generatedFiles = $providerList | ForEach-Object {
	$xmlOutputFolder = 'C:\temp\Providers'
	$providerName = $_
	$providerXmlFilename = $providerName + '.xml'
	$outFile = Join-Path -Path $xmlOutputFolder -ChildPath $providerXmlFilename
	$wepToolArgs = "/meta /eventmeta /name '$providerName' /out '$outFile'"
	$executeWepTool = "$wepTool $wepToolArgs"
	
	# Test if the output folder exists, and create it if necessary
	if (-not (Test-Path -Path $xmlOutputFolder -PathType Container))
	{
		$null = New-Item -Path $xmlOutputFolder -ItemType Directory
	}
	
	# Measure execution time
	$timer = Measure-Command {
		# Execute the external command-line tool
		Invoke-Expression $executeWepTool
	}
	
	# Output a message indicating the XML file created and the execution time
	Write-Host "Created XML file: $providerXmlFilename ($($timer.TotalSeconds) seconds)"
	$providerXmlFilename
}

# Generate All.xml to complete the set of what's produced when running the GUI
$allWepToolArgs = "/out c:\temp\All.xml"
$executeWepToolAll = "$wepTool $allWepToolArgs"

# Measure execution time for generating All.xml
$allTimer = Measure-Command {
	# Execute the external command-line tool to generate All.xml
	Invoke-Expression $executeWepToolAll
}

# Output a message indicating the XML file created and the execution time for generating All.xml
Write-Host (Get-Date).ToString() "| Created All.xml ($($allTimer.TotalSeconds) seconds)"

# Delete files under 30 bytes
$xmlOutputFolder = 'C:\temp\Providers'
$filesToDelete = Get-ChildItem -Path $xmlOutputFolder -File | Where-Object { $_.Length -lt 30 }
$deletedFileCount = $filesToDelete.Count

if ($generatedFiles.Count -gt 0)
{
	# Output a message showing the count of XML files generated
	Write-Host (Get-Date).ToString() "| Total XML files generated: $($generatedFiles.Count)"
}
...
if ($deletedFileCount -gt 0)
{
	...
	# Output a message indicating the deletion and the count of deleted files
	Write-Host (Get-Date).ToString() "| Deleted $deletedFileCount files under 30 bytes"
	
	# Calculate the count of remaining files
	$remainingFiles = Get-ChildItem -Path $xmlOutputFolder -File | Where-Object { $_.Length -ge 30 }
	$remainingFileCount = $remainingFiles.Count
	
	# Output a summary of the remaining files and the number of files deleted
	Write-Host (Get-Date).ToString() "| Summary:"
	Write-Host (Get-Date).ToString() "|   Files Remaining: $remainingFileCount"
	Write-Host (Get-Date).ToString() "|   Files Deleted:   $deletedFileCount"
}
else
{
	Write-Host (Get-Date).ToString() "| No files found under 30 bytes to delete"
}

# SIG # Begin signature block
# MIIviwYJKoZIhvcNAQcCoIIvfDCCL3gCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC