<#
	.DESCRIPTION
		A PowerShell 5 script that can be used to convert MBAMSERVICE.LOG* files into CSV
	
	.PARAMETER path
		Point to a path where MBAMSERVICE.LOG* files exist
	
	.PARAMETER outputPath
		Point to where you want the CSV output to go
	
	.NOTES
		===========================================================================
		Created with: 	SAPIEN Technologies, Inc., PowerShell Studio 2023 v5.8.223
		Created on:   	2023-06-05 10:38
	...
	.EXAMPLE
		...
		01/07/22	" 07:31:54.595"	1841239390	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::GetInstalledPkgVersions	"updatecontrollerimplhelper.cpp"	1631	"UI package --> [mbam-c.ui.64bit], current version: [4.0.13]"
		01/07/22	" 07:31:54.897"	1841239656	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::DoUpdate	"updatecontrollerimplhelper.cpp"	887	"Checked for updates - no updates available"
		01/07/22	" 07:31:54.890"	1841239656	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::DoUpdate	"updatecontrollerimplhelper.cpp"	926	"Update check is complete."
		01/07/22	" 07:31:54.890"	1841239656	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::ResetUpdateState	"updatecontrollerimplhelper.cpp"	936	"Setting thread priority to normal"
		01/07/22	" 08:31:53.894"	1847481236	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::ValidateDBManifest	"updatecontrollerimplhelper.cpp"	5629	"Signature successfully validated"
		01/07/22	" 08:31:54.592"	1847481238	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::ValidateDBManifest	"updatecontrollerimplhelper.cpp"	5633	"DB manifest successfully validated"
		01/07/22	" 08:31:54.592"	1847481238	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::CheckDbManifest	"updatecontrollerimplhelper.cpp"	5984	"Validated DB manifest - success"
		01/07/22	" 08:31:54.592"	1847481238	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::DoUpdate	"updatecontrollerimplhelper.cpp"	829	"DoUpdate - Starting check for updates (automatic)"
		01/07/22	" 08:31:54.592"	1847481238	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::DoUpdate	"updatecontrollerimplhelper.cpp"	839	"Checking for: Installer=[Yes], CU/SU/UI=[Yes], DB/CLS=[No]"
		01/07/22	" 08:31:54.593"	1847481238	17d4	6a79	INFO	UpdateControllerImpl	mb::updatecontrollerimpl::CUpdateControllerImpl::GetInstalledPkgVersions	"updatecontrollerimplhelper.cpp"	1546	"Installer package --> [mbam-c.installer.consumer], current version: [4.5.19]"
#>
param (
...
$logFiles = Get-ChildItem -Path $path -Filter "MBAMSERVICE.LOG*" | Where-Object { -not $_.Name.EndsWith(".csv") }

# Count the log files
$logFileCount = $logFiles.Count
Write-Host "Number of MBAMSERVICE.LOG files found: $logFileCount"

# Display the filenames with extensions
...
foreach ($logFile in $logFiles)
{
	...
	# Get the content of the log file and skip the first line if it's a header
	$content = (Get-Content $logFile.FullName) | Select-Object -Skip 1
	
	# Calculate inputRowCount and inputFileSize
	$inputRowCount = $content.Count
	$inputFileSize = (Get-Item $logFile.FullName).Length
	
	# Define an empty array to store the converted data
	$csvData = @()
	
	# Define the name of the output CSV file (replace "." with "_" and append ".csv")
	$outFile = $logFile.Name.Replace('.', '_') + ".csv"
	
	# Check if $outputFolder is not specified or empty
	if (-not $outputFolder)
	{
		$outputPath = Join-Path -Path $path -ChildPath $outFile
		Write-Host "Output will be generated here: $outputPath"
	}
	else
	{
		# Check if $outputFolder is a valid directory (container)
		if (-not (Test-Path -Path $outputFolder -PathType Container))
		{
			# If the directory does not exist, create it
			New-Item -ItemType Directory -Force -Path $outputFolder | Out-Null
			Write-Host "The output folder '$outputFolder' did not exist and has been created."
		}
		$outputPath = Join-Path -Path $outputFolder -ChildPath $outFile
		Write-Host "Output will be generated here: $outputPath"
	}
	
	# Define the name of the excluded output CSV file (replace "." with "_" and append ".csv")
	$excludedOutFile = $logFile.Name.Replace('.', '_') + "_excluded.txt"
	
	# Check if $outputFolder is not specified or empty
	if (-not $outputFolder)
	{
		$excludedFilePath = Join-Path -Path $path -ChildPath $excludedOutFile
		Write-Host "Any excluded output will be generated here: $excludedFilePath"
	}
	else
	{
		$excludedFilePath = Join-Path -Path $outputFolder -ChildPath $excludedOutFile
		Write-Host "Any excluded output will be generated here: $excludedFilePath"
	}
	
	# Create an empty array to store the excluded lines
	$excludedLines = @()
	
	# Iterate over each line in the file
	foreach ($line in $content)
	{
		# Split the line on tab character to get the columns
		$columns = $line -split "`t"
		
		# Skip lines that do not have 11 columns (the expected number based on the provided header)
		if ($columns.Count -eq 11)
		{
			# Create a new object for the line
			$csvLine = New-Object PSObject
			
			# Convert the date format to 'yyyy/MM/dd HH:mm:ss.fff'
			# Trim extra spaces and remove the quotes
			$timestampString = ($columns[0].Trim() + " " + $columns[1].Replace('"', '').Trim())
			
			# Attempt to parse the datetime with 4 milliseconds digits
			try
			{
				$timestamp = [datetime]::ParseExact($timestampString, "MM/dd/yy HH:mm:ss.ffff", $null)
			}
			# If parsing with 4 milliseconds digits fails, then attempt to parse with 3 milliseconds digits
			catch
			{
				try
				{
					$timestamp = [datetime]::ParseExact($timestampString, "MM/dd/yy HH:mm:ss.fff", $null)
				}
				catch
				{
					Write-Host "Unable to parse datetime: $timestampString"
				}
			}
			
			$formattedTimestamp = $timestamp.ToString('yyyy/MM/dd HH:mm:ss.fff')
			
			# Add properties to the object
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Timestamp" -Value $formattedTimestamp
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Tick Count" -Value $columns[2]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Process ID" -Value $columns[3]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Thread ID" -Value $columns[4]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Log Level" -Value $columns[5]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Context Tag" -Value $columns[6]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Function Name" -Value $columns[7]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "File Name" -Value $columns[8]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Line Number" -Value $columns[9]
			Add-Member -InputObject $csvLine -MemberType NoteProperty -Name "Message" -Value $columns[10]
		...
		{
			# Add the excluded line to the array
			$excludedLines += $line
		}
	}
	...
	# Write excluded lines to the excluded.txt file
	$excludedLines | Out-File -FilePath $excludedFilePath -Encoding UTF8
	
	# Calculate the number of rows in the output .csv file and its size
	$outputRowCount = $csvData.Count
	$outputFileSize = (Get-Item $outputPath).Length
	
	# Display the difference between the row counts and file sizes
	Write-Host "=============================================================="
	Write-Host "For input file: $($logFile.Name)"
	Write-Host "Input row count: $inputRowCount"
	Write-Host "Input file size: $inputFileSize bytes, $($inputFileSize / 1KB -as [int]) KB"
	Write-Host "For output file: $outFile"
	Write-Host "Output row count: $outputRowCount"
	Write-Host "Output file size: $outputFileSize bytes, $($outputFileSize / 1KB -as [int]) KB"
	Write-Host "Difference in row count: $(($inputRowCount - $outputRowCount))"
	Write-Host "Difference in file size: $(($inputFileSize - $outputFileSize)) bytes, $((($inputFileSize - $outputFileSize) / 1KB) -as [int]) KB"
	# Write excluded lines to the excluded.txt file
	$excludedLines | Out-File -FilePath $excludedFilePath -Encoding UTF8
	...
	# Check if there are excluded lines
	if ($excludedLines)
	{
		Write-Host "--------------------------------------------------------------"
		Write-Host "Excluded lines for: $($logFile.Name)"
		
		# Display the number of excluded lines
		Write-Host "Number of excluded lines: $($excludedLines.Count)"
		Write-Host "=============================================================="
	}
	else
	{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAauH18Zthk40pj
# SN6D2FEqbFqSJKZSMyKBzhWmyWlujKCCKJAwggQyMIIDGqADAgECAgEBMA0GCSqG
...
# CisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSI